...
import { AlloyResult } from "./alloy-result";
...
class Alloy {
    ...
    setJar (jar) {
        ...
        if (jar && jar.split('.').pop() === 'jar') {
            ...
            if (this._alloy) {
                ...
                this._emitter.emit('info', new AlloyInfo(null, 'Alloy', 'Successfully connected to Alloy subprocess'));
                this._dequeue();

            } else {

                this._emitter.emit('error', new AlloyError(null, 'Alloy', 'Unable to connect to Alloy subprocess'));

            }

        } else {

            this._emitter.emit('error', new AlloyError(null, 'JAR', 'Invalid .jar file'));
    ...
    onResult (callback) {
        return this._emitter.on('result', callback);
    }
    ...
    _onErr (data) {
        ...
        this._emitter.emit('error', new AlloyError(this._parser.file, 'Error', `${data}`));
    ...
    _onParse (item) {
        ...
        if (item instanceof AlloyCommandList)
            this._emitter.emit('list', item);

        if (item instanceof AlloyResult)
            this._emitter.emit('result', item);
    ...
    _onExit (code, signal) {
        ...
        this._emitter.emit('info', new AlloyInfo(this._parser.file, 'Exit', `Alloy exited with code ${code}`));
    ...
    _restart () {
        ...
        this._emitter.emit('info', new AlloyInfo(this._parser.file, 'Restart', 'Alloy Restarted'));
    ...
    _parse () {
        ...
        if (this._currLines.length > 0) {
            ...
            switch (this._currLines[0]) {
                ...
                case 'c':
                    this._post(new AlloyCommandList(this.file, this._currLines));
                    break;

                case 'm':
                    if (this._currLines.length === 2) {
                        this.file = this._currLines[1];
                        this._post(new AlloyInfo(this.file, 'Model Set', this.file));
                    } else {
                        this._post(new AlloyError(this.file, 'Model Set', 'Error parsing model set command'));
                    }
                    break;

                case 'r':
                    this._post(new AlloyResult(this.file, this._currLines));
                    break;
